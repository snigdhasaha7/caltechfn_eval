...
class SVHNDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_dir, ann_file, transform=None, target_transform=None, remove_empty=False):
        ...
        if self.remove_empty:
            ...
            self.ids = list(self.svhn.imgToAnns.keys())
        else:
            # when testing, all images used.
            self.ids = list(self.svhn.imgs.keys())
        svhn_categories = sorted(self.svhn.getCatIds())
        self.coco_id_to_contiguous_id = {coco_id: i + 1 for i, coco_id in enumerate(svhn_categories)}
    ...
    def _get_annotation(self, image_id):
        ann_ids = self.svhn.getAnnIds(imgIds=image_id)
        ann = self.svhn.loadAnns(ann_ids)
    ...
    def get_img_info(self, index):
        image_id = self.ids[index]
        img_data = self.svhn.imgs[image_id]
        return img_data

    def _read_image(self, image_id):
        file_name = self.svhn.loadImgs(image_id)[0]['file_name']
